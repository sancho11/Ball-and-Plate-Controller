...
/*******************************************************************************
* Function Name: ClockServoControl_Start
********************************************************************************
*
* Summary:
...
void ClockServoControl_Start(void) 
{
    ...
    ClockServoControl_CLKEN |= ClockServoControl_CLKEN_MASK;
	ClockServoControl_CLKSTBY |= ClockServoControl_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ClockServoControl_Stop
********************************************************************************
*
* Summary:
...
void ClockServoControl_Stop(void) 
{
    ...
    ClockServoControl_CLKEN &= (uint8)(~ClockServoControl_CLKEN_MASK);
	ClockServoControl_CLKSTBY &= (uint8)(~ClockServoControl_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ClockServoControl_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ClockServoControl_CLKEN & ClockServoControl_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ClockServoControl_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ClockServoControl_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ClockServoControl__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ClockServoControl_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ClockServoControl_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ClockServoControl_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ClockServoControl_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ClockServoControl_GetSourceRegister();
    uint16 oldDivider = ClockServoControl_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ClockServoControl_CLKEN & ClockServoControl_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ClockServoControl_DIV_PTR, clkDivider);
                ClockServoControl_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ClockServoControl_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ClockServoControl_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ClockServoControl_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ClockServoControl_DIV_PTR, clkDivider);
				ClockServoControl_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ClockServoControl_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ClockServoControl_DIV_PTR);
}


/*******************************************************************************
* Function Name: ClockServoControl_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockServoControl_MOD_SRC |= modeBitMask & (uint8)ClockServoControl_MODE_MASK;
}


/*******************************************************************************
* Function Name: ClockServoControl_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockServoControl_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ClockServoControl_MODE_MASK));
}


/*******************************************************************************
* Function Name: ClockServoControl_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ClockServoControl_MOD_SRC & (uint8)(ClockServoControl_MODE_MASK);
}


/*******************************************************************************
* Function Name: ClockServoControl_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ClockServoControl_GetDividerRegister();
    uint8 oldSrc = ClockServoControl_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ClockServoControl_MOD_SRC |= CYCLK_SSS;
        ClockServoControl_MOD_SRC =
            (ClockServoControl_MOD_SRC & (uint8)(~ClockServoControl_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ClockServoControl_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ClockServoControl_MOD_SRC & ClockServoControl_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ClockServoControl_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ClockServoControl_PHASE = clkPhase & ClockServoControl_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ClockServoControl_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ClockServoControl_PHASE & ClockServoControl_PHASE_MASK;
}
