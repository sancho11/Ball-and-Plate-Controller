/*******************************************************************************
* File Name: Touch.c
* Version 2.0
*
* Description:
*  This file provides the API source code for the Resistive Touch component.
*
* Note:
*
********************************************************************************
* Copyright 2012-2015, Cypress Semiconductor Corporation.  All rights reserved.
* You may use this file only in accordance with the license, terms, conditions,
* disclaimers, and limitations in the end user license agreement accompanying
* the software package with which this file was provided.
*******************************************************************************/

#include "CyLib.h"
#include "Touch.h"
#include "Touch_xm.h"
#include "Touch_xp.h"
#include "Touch_ym.h"
#include "Touch_yp.h"

uint8 Touch_initVar = 0u;
static uint8 Touch_measurementFlag = 0u;


/*******************************************************************************
* Function Name: Touch_Init
********************************************************************************
*
* Summary:
*  Calls the Init() function of the ADC and AMux components.
*
* Parameters:
*  None
*
* Return:
*  None
*
*******************************************************************************/
void Touch_Init(void) 
{
    Touch_AMux_Init();
    Touch_ADC_Init();
}


/*******************************************************************************
* Function Name: Touch_Start
********************************************************************************
*
* Summary:
*  Calls Init() if the component has not been initialized before. Calls Enable()
*  to begin the component operation.
*
* Parameters:
*  None
*
* Return:
*  None
* 
* Global variables:
*  Touch_initVar - Indicates whether the component has been
*                             initialized.
*
*******************************************************************************/
void Touch_Start(void)  
{
    if(Touch_initVar == 0u)
    {    
        Touch_Init();
        Touch_initVar = 1u;
    }
    Touch_Enable();
}


/*******************************************************************************
* Function Name: Touch_Stop
********************************************************************************
*
* Summary:
*  Stops the ADC and AMux components.
*
* Parameters:
*  None
*
* Return:
*  None
*
*******************************************************************************/
void Touch_Stop(void)  
{
    Touch_ADC_Stop();
    Touch_AMux_Stop();
}


/*******************************************************************************
* Function Name: Touch_Enable
********************************************************************************
*
* Summary:
*  Enables the ADC component.
*
* Parameters:
*  None
*
* Return:
*  None
*
*******************************************************************************/
void Touch_Enable(void)  
{
    Touch_ADC_Enable();
}


/*******************************************************************************
* Function Name: Touch_ActivateX
********************************************************************************
*
* Summary: 
*  Configures the pins to measure the Y-axis.
*
* Parameters:
*  None
*
* Return:
*  None
*
*******************************************************************************/
void Touch_ActivateX(void) 
{
    /* Pins configuration to measure Y. */
    CyPins_SetPinDriveMode(Touch_ym_0, PIN_DM_STRONG);
    CyPins_SetPinDriveMode(Touch_yp_0, PIN_DM_STRONG);
    CyPins_SetPinDriveMode(Touch_xp_0, PIN_DM_ALG_HIZ);
    CyPins_SetPinDriveMode(Touch_xm_0, PIN_DM_ALG_HIZ);
    
    /* Switch AMux if channel 0 is not selected. */
    if(Touch_AMux_GetChannel() != Touch_AMUX_XP_CHAN)
    {
        Touch_AMux_Next();
    }
}


/*******************************************************************************
* Function Name: Touch_ActivateY
********************************************************************************
*
* Summary: 
*  Configures the pins to measure X-axis.
*
* Parameters:
*  None
*
* Return:
*  None
*
*******************************************************************************/
void Touch_ActivateY(void) 
{
    /* Pins configuration to measure X. */
    CyPins_SetPinDriveMode(Touch_xm_0, PIN_DM_STRONG);
    CyPins_SetPinDriveMode(Touch_xp_0, PIN_DM_STRONG);
    CyPins_SetPinDriveMode(Touch_yp_0, PIN_DM_ALG_HIZ);
    CyPins_SetPinDriveMode(Touch_ym_0, PIN_DM_ALG_HIZ);
    
    /* Switch AMux if channel 1 is not selected. */
    if(Touch_AMux_GetChannel() == Touch_AMUX_XP_CHAN)
    {
        Touch_AMux_Next();
    }
    /* Switch AMux to channel 1 if AMux is disconnected. */
    else if(Touch_AMux_GetChannel() == Touch_AMUX_NO_CHAN)
    {
        Touch_AMux_Next();
        Touch_AMux_Next();
    }
    else
    {
        /* Channel 1 is already selected. */
    }
}


/*******************************************************************************
* Function Name: Touch_TouchDetect
********************************************************************************
*
* Summary:
*  Determines if the screen is touched.
*
* Parameters:
*  None
*
* Return:
*  uint8: The touch state.
*  0 - untouched
*  1 - touched
*
*******************************************************************************/
uint8 Touch_TouchDetect(void) 
{
    /* Configure x+ pin to detect touch. */
    CyPins_SetPinDriveMode(Touch_xp_0, PIN_DM_STRONG);

    /* Add delay for signal to stabilize. */
    CyDelayUs(5u);

    CyPins_SetPinDriveMode(Touch_xp_0, PIN_DM_RES_UP);
    
    /* Switch AMux if channel 0 is not selected. */
    if(Touch_AMux_GetChannel() != Touch_AMUX_XP_CHAN)
    {
        Touch_AMux_Next();
    }

    /* Configure x-, y+ and y- to detect touch. */
    CyPins_SetPinDriveMode(Touch_xm_0, PIN_DM_ALG_HIZ);
    CyPins_SetPinDriveMode(Touch_yp_0, PIN_DM_ALG_HIZ);
    CyPins_SetPinDriveMode(Touch_ym_0, PIN_DM_STRONG);

    /* Add delay for signal to stabilize. The delay is dependant on whether
    * the measurement has been done before touch detect or not.
    */
    if (Touch_measurementFlag == 0u)
    {
        CyDelayUs(20u);
    }
    else
    {
        Touch_measurementFlag = 0u;
        CyDelayUs(8u);        
    }
    /* The screen is touched if the measured value is lower than the specified
    * threshold.
    */
    return ((Touch_Measure() < Touch_TOUCH_THRESHOLD) ? 1u : 0u);
}


/*******************************************************************************
* Function Name: Touch_Measure
********************************************************************************
*
* Summary:
*  Returns the result of the ADC conversion. Does not return until ADC
*  conversion is complete.
*
* Parameters:
*  None
*
* Return:
*  int16: the result of the ADC conversion.
*
*******************************************************************************/
int16 Touch_Measure(void) 
{       
    int16 result;

    /* Trigger the ADC conversion and wait for results (blocking). */
    Touch_ADC_StartConvert();
    (void)Touch_ADC_IsEndConversion(Touch_ADC_WAIT_FOR_RESULT);
    result = Touch_ADC_GetResult16();

    /* Assert a flag indicating that the measurement has been done. */
    Touch_measurementFlag = 1u;
    return (result);
}


/* [] END OF FILE */
