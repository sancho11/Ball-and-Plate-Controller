/*******************************************************************************
* File Name: Potenciometro_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Potenciometro_IntClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Potenciometro_IntClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    Potenciometro_IntClock_CLKEN |= Potenciometro_IntClock_CLKEN_MASK;
	Potenciometro_IntClock_CLKSTBY |= Potenciometro_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Potenciometro_IntClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Potenciometro_IntClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Potenciometro_IntClock_CLKEN &= (uint8)(~Potenciometro_IntClock_CLKEN_MASK);
	Potenciometro_IntClock_CLKSTBY &= (uint8)(~Potenciometro_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Potenciometro_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Potenciometro_IntClock_StopBlock(void) 
{
    if ((Potenciometro_IntClock_CLKEN & Potenciometro_IntClock_CLKEN_MASK) != 0u)
    {
...
#if defined(Potenciometro_IntClock__CFG3)
        CLK_DIST_AMASK = Potenciometro_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Potenciometro_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Potenciometro_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Potenciometro_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Potenciometro_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Potenciometro_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Potenciometro_IntClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Potenciometro_IntClock_CLKSTBY &= (uint8)(~Potenciometro_IntClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Potenciometro_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Potenciometro_IntClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Potenciometro_IntClock_GetSourceRegister();
    uint16 oldDivider = Potenciometro_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Potenciometro_IntClock_CLKEN & Potenciometro_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Potenciometro_IntClock_DIV_PTR, clkDivider);
                Potenciometro_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Potenciometro_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Potenciometro_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Potenciometro_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Potenciometro_IntClock_DIV_PTR, clkDivider);
				Potenciometro_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Potenciometro_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Potenciometro_IntClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(Potenciometro_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Potenciometro_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Potenciometro_IntClock_SetModeRegister(uint8 modeBitMask) 
{
    Potenciometro_IntClock_MOD_SRC |= modeBitMask & (uint8)Potenciometro_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Potenciometro_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Potenciometro_IntClock_ClearModeRegister(uint8 modeBitMask) 
{
    Potenciometro_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Potenciometro_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Potenciometro_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Potenciometro_IntClock_GetModeRegister(void) 
{
    return Potenciometro_IntClock_MOD_SRC & (uint8)(Potenciometro_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Potenciometro_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Potenciometro_IntClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Potenciometro_IntClock_GetDividerRegister();
    uint8 oldSrc = Potenciometro_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Potenciometro_IntClock_MOD_SRC |= CYCLK_SSS;
        Potenciometro_IntClock_MOD_SRC =
            (Potenciometro_IntClock_MOD_SRC & (uint8)(~Potenciometro_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Potenciometro_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Potenciometro_IntClock_GetSourceRegister(void) 
{
    return Potenciometro_IntClock_MOD_SRC & Potenciometro_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Potenciometro_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Potenciometro_IntClock_SetPhaseRegister(uint8 clkPhase) 
{
    Potenciometro_IntClock_PHASE = clkPhase & Potenciometro_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Potenciometro_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Potenciometro_IntClock_GetPhaseRegister(void) 
{
    return Potenciometro_IntClock_PHASE & Potenciometro_IntClock_PHASE_MASK;
}
